#!/usr/bin/env python3
"""Local, read-only SSE NIF surface picker and portable selection projects.

This is an authoring preview, not an add-on exporter. The original NIF is
retained byte-for-byte. No mesh naming, registration or game files are edited.
"""
...
PROJECT_FORMAT = "mirrors-of-skyrim/surface-selection"
...
class Document:
    ...
    def public(self):
        return dict(id=self.digest, name=self.name, bytes=len(self.data),
                    profile="Skyrim SE / VR · static BSTriShape", warnings=self.warnings,
...
def load_nif(data: bytes, name="mirror.nif") -> Document:
    ...
    try:
        ...
        require(version == 0x14020007 and endian == 1 and user == 12 and stream == 100,
                "This preview supports Skyrim SE / VR NIFs (stream 100). Convert an LE or other-game NIF to SE first.")
        ...
        def walk(index, parent_t=(0., 0., 0.), parent_r=IDENTITY, parent_s=1., hidden=False, depth=0):
            ...
            require(kind in ('NiNode', 'BSFadeNode', 'BSTriShape'),
                    f"This preview cannot read scene part {kind}. Use a static SE NIF with BSTriShape geometry.")
            ...
            t = values[:3]; r = tuple(tuple(values[3+i*3:6+i*3]) for i in range(3)); s = values[12]
            require(1e-7 <= abs(s) <= 1e7, "The NIF contains a collapsed or excessive scale.")
            ...
            require(skin == -1 and not flags & 0x40, f'“{part_name}” is skinned. Use an unskinned mirror model for this preview.')
            require(flags & 1 and stride >= 16 and not (desc & 0xf0), f'“{part_name}” has unsupported or missing vertex positions.')
            require(nv > 0 and nt > 0, f'“{part_name}” contains no triangles.')
            require(size == nv*stride + nt*6 and len(block) >= off+44+size+4,
                    f'“{part_name}” has inconsistent vertex or triangle data.')
            require(len(block) == off+44+size+4 and struct.unpack_from('<I', block, off+44+size)[0] == 0,
                    f'“{part_name}” contains particle data unsupported by this preview.')
            pos = []
            for v in range(nv):
                local = struct.unpack_from('<3f', block, off+44+v*stride)
            ...
            # CK's zero-based 3D Index follows geometry in scene-child order,
            # including hidden shapes. It is not a NIF block/string index or
            # the numeric suffix of a shape name. Unsupported scene parts are
            # rejected above rather than skipped and silently misnumbered.
        ...
        if any(m.hidden for m in meshes):
            warnings.append('Some parts are hidden in the NIF. Use their visibility buttons to inspect them.')
...
def load_project(data: bytes):
    ...
    try:
        with zipfile.ZipFile(io.BytesIO(data)) as archive:
            ...
            require(project.get('mirrorType') in ('hand', 'placeable') and project.get('playableAddon') is False, 'The project has an unsupported mirror profile.')
