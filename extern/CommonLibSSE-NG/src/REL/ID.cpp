...
namespace REL
{
	...
	bool IDDatabase::load_file(stl::zwstring a_filename, Version a_version, std::uint8_t a_formatVersion, bool a_failOnError)
	{
		try {
			...
			// A format-5 table is laid out differently from what other plugins'
			// CommonLib builds may have published under the shared v2 name.
			auto mapname = header.flat_table() ? L"MirrorsOfSkyrimOffsets-v5-"s :
