...
#include "REL/FixedRuntimeAddresses.h"
...
namespace REL
{
	...
	{
	private:
		using mapping_t = FixedRuntimeAddresses::Entry;
	...
	public:
		...
		public:
			...
			{
				const std::span<const mapping_t> id2offset = IDDatabase::get().entries();
...
#	endif
		...
		[[nodiscard]] inline std::size_t id2offset(std::uint64_t a_id) const
		{
			...
			const bool failed = it == mappings.end() || it->id != a_id;
			if (failed) {
				stl::report_and_fail(
					std::format(
						"Missing engine address ID: {}\n"
						"This plugin has no verified address for this ID on the "
						"selected executable version."sv,
						a_id));
			}
	...
	private:
		...
		public:
			void read(istream_t& a_in, std::uint8_t a_formatVersion)
			{
				std::int32_t format{};
				a_in.readin(format);
				// Address Library format 5 (Skyrim 1.7.x): four version dwords, a
				// fixed 64-byte module name, pointer size, a reserved dword, the
				// entry count, then a flat little-endian uint32 offset per ID
				// (0 = unmapped).  The ID space is the AE one (ID 1 -> 0x1022 in
				// both 1.6.1170 and 1.7.104 databases).
				if (format == 5 && a_formatVersion == 2) {
					std::int32_t version5[4]{};
					a_in.readin(version5);
					a_in.ignore(64);
					a_in.readin(_pointerSize);
					std::int32_t reserved{};
					a_in.readin(reserved);
					a_in.readin(_addressCount);
					for (std::size_t i = 0; i < std::extent_v<decltype(version5)>; ++i) {
						_version[i] = static_cast<std::uint16_t>(version5[i]);
					}
					_flatTable = true;
					return;
				}
			...
			[[nodiscard]] bool flat_table() const noexcept { return _flatTable; }
		...
		[[nodiscard]] std::span<const mapping_t> entries() const noexcept
		{
			return _fixedOffsets.empty() ? std::span<const mapping_t>{_id2offset} : _fixedOffsets;
		}

		void load()
		{
			const auto version = Module::get().version();
			_fixedOffsets = FixedRuntimeAddresses::Select({version[0], version[1], version[2], version[3]});
			if (_fixedOffsets.empty()) {
				stl::report_and_fail("This executable version has no verified built-in engine addresses.");
			}
		}
		...
		{
			if (a_header.flat_table()) {
				// Format 5: entry i is the offset of ID i.  Unmapped IDs sort to the
				// end so a lookup fails loudly instead of resolving to the module base.
				std::uint64_t index = 0;
				for (auto& mapping : _id2offset) {
					const auto value = a_in.readout<std::uint32_t>();
					if (value != 0) {
						mapping = { index, static_cast<std::uint64_t>(value) };
					} else {
						mapping = { ~std::uint64_t{ 0 }, 0 };
					}
					++index;
				}
				return true;
		...
		std::span<const mapping_t>     _fixedOffsets;
